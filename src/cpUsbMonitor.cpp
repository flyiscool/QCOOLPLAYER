...
extern CPThreadUsbMonitor* g_thUsbMonitor;
...
void UpdateDevice(PDEV_BROADCAST_DEVICEINTERFACE pDevInf, WPARAM wParam)
{
	...
	if (DBT_DEVICEARRIVAL == wParam) {
		bat = true;
		g_thUsbMonitor->DoemitSingal(bat);
	}
...
void CPThreadUsbMonitor::DoemitSingal(bool arrival)
{
	emit sig_USBUpdate(arrival);
	qDebug() << "hotplug "  << arrival  << endl;
}